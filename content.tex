...
{\color{blue!50!cyan}
\begin{compactitem}
	\item For the \textit{audio encoders} \jumper and \flac, the measured task was to encode raw WAVE audio signals to MP3 (\jumper) and FLAC format (\flac). We selected a number of different audio files from the Wikimedia Commons collection and aimed at varying the file size/signal length, sampling rate, and number of channels. Both applications share all workloads.

\item For the \textit{video encoders} \xzwo and \vpnine, the measured task was to encode raw video frames (y4m format). We selected a number of files from xiph.org’s “derf collection”, a set of test media for a variety of use cases. The frame files vary in resolution (low/SD up to 4K) and file size. Both applications in this domain were tested with the same workload set. 
...
\begin{table}
	...
	\footnotesize
	\centering
	\caption{\color{blue!50!cyan}Hardware specifications of the compute clusters used.}
	\label{tab:hardware}
...
\begin{tabular}{lrrrrrrrr}	
	...
	\jadx & 
	0 & 0\,\% & 
	\cellcolor{lt-color!0!white}0 & \cellcolor{lt-color!0!white}0.0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
	...
	\rowcolor{blue!25}	\lrzip & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
	
	\rowcolor{blue!25}	\flac & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
	
	\rowcolor{blue!25}	\vpnine & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
	
	\rowcolor{blue!25}	\xzwo & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
	
	\rowcolor{blue!25}	\zdrei & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\% & 
	0 & 0\,\%\\
...
{\color{cyan}
Threats to \textbf{internal validity} include measurement noise which may distort our classification into categories (Section~\ref{sec:rq1}) and model construction (Section~\ref{sec:rq2}). We mitigate these threats by repeating each experiment five times and reporting the median as a robust measure in a controlled environment. Moreover, the coverage analysis (cf. Section ?) entails a noticeable instrumentation overhead, which may distort performance observations. We mitigate this threat by separating the experiment runs for coverage assessment and performance measurement. In the case of h2, the load generator of the \textsc{OLTPBench} framework~\cite{difallah_oltp_2013} ran on the same machine as the database since we were testing an embedded scenario with only negligible overhead. 	

Threats to\textbf{ external validity} include the selection of subject systems and workloads. Regarding the subject systems, we address this issue by selecting software from various application domains as well as two different programming language ecosystems (cf.~Table~\ref{tab:subject_systems}). In lieu of domain knowledge, we cannot select workloads systematically with respect to workload characteristics due to workloads inherent opacity. We address this issue by varying likely relevant characteristics and, to some extent, reusing workloads across subject systems of the same domain. Achieving true representativeness is desirable, yet intractable. As the goal of this selection is to study the presence and quality of workload-option interactions, not their prevalence, we believe this selection does not invalidate our findings. Nonetheless, one cannot necessarily conclude that our results hold for other domains and programming languages.
}
