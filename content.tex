...
For code sections that are not covered by execution under a specific workload, one cannot determine the influence they might have on performance under different workloads. This leaves performance models susceptible to bias by the choice of the workload, so that a model can over-approximate during learning and may not generalize to real-world use cases.
...
If the execution of performance-relevant code is determined only by a specific workload, one cannot measure the influence such code would contribute to performance otherwise.
...
Ich kann dir bis hier hin folgen. Allerdings sind die letzten 2 Sätze wohl für jemanden, der jetzt nicht im Thema ist, schwerer nachzuvollziehen. 
Evtl. wäre hier ein kleines Code-Snippet gut, in welchem (1) ein bestimmter featureunabhängiger Code durch einen bestimmten Workload abgedeckt wird und (2) featureabhängiger Code (also sozusagen Code der einem Feature zugeordnet wird) in welchem Teile davon von einem spezifischen Workload abgedeckt werden. 
Featureabhängigen Code könnte man über normale ifs realisieren. 
if (feature-a == true) \\
Workloadabhängigen Code könnte man beispielsweise im Datenbankkontext so realisieren:
if (inserted-row-count > 50) 
Also wenn sozusagen mehr als 50 Reihen in eine Tabelle einer DAtenbank eingefügt wurden, passiert was (z.B. Optimierungen). 
NAchtrag: Ach, ich sehe jetzt erst Listing 1. Könnte man das nicht in verkleinerter Form (also beispielsweise durch ein anderes Szenario) bereits auf Seite 1 oder 2 drankommen lassen?
Man könnte das ganze Szenario auch so abändern, dass in den farblich markierten Featureregionen irgendwelche Funktionsaufrufe drinnen sind, die andeuten, viel Performance zu brauchen. Dann würde man zumindest ein wenig besser illustrieren, dass das einen großen Effekt auf die Performance haben kann.
} 
...
\pdfcomment[color=green!25]{
Hier könnte man dann auch wieder auf das Beispiel in Listing 1 verweisen, um das Verständnis zusätzlich zu erhöhen.
}
...
\sosy{jump3r}\footnote{\url{https://github.com/Sciss/jump3r/}} is a re-implementation of the LAME audio codec for MP3 in Java. In total, we have selected {\color{red}six} WAVE audio files as a workload to encode to MP3. While the selection of workloads was exploratory, we indcluded audio files with different characteristics, such as sampling rate, number of channels, and length. 
\sosy{kanzi}\footnote{\url{https://github.com/flanglet/kanzi/}} is a file compression tool. For this subject system, we selected {\color{red}ten} workloads, including, among others, dedicated compression benchmarks (sets of files of different types) as well as binary and text files at different scales: a binary of the Linux kernel, a dump of its repository, XML and CSV data. 
...
For \sosy{jadx}, we sampled 30 Android applications from AndroZoo~\cite{allix_androzoo_2016}
\noindent
...
All experiments were conducted on compute clusters with an identical hardware setup, either Intel~Core~i5-8259U CPUs at 2.3~GHz (\sosy{jump3r}, \sosy{kanzi}, \sosy{jump3r}, \sosy{h2}, and \sosy{hsqldb}) or Intel~Core~i7-8559U CPUs at 2.7~GHz (\sosy{density-converter}, \sosy{batik}, and \sosy{jadx}) and 32~GB of RAM, respectively. The machines ran a headless Debian 10 installation (kernel version \mbox{4.19.0-14}). To minimize measurement noise, no additional user process were running in the background and no other than necessary packages were installed.
...
\greybox{\textit{Summary:} This appears to work, but we do not know how and why yet.}
