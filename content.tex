...
While one might first anticipate that a more complex workload results in lower throughput, this is correct, but does not hold for all configurations. We see that the median throughput indeed decreases when increasing the scale factor, however the shape and spread of the performance distribution is affected as well. While the majority of configurations’ performance scales with the workload, some do not and even achieve better throughput. That is, if configurations’ performance is affected in such non-trivial ways, varying the workload affects the performance influence that different configuration options exhibit. 

Some aspects of this \emph{input sensitivity} has been observed and documented before in literature~\cite{liao_2020_using_emse,alves_sampling_2020,jamishidi_transfer_2017} and raises the questions, such as: Which options are input sensitive? What workload characteristics affect an option’s performance-influence? Can we estimate which options are input-sensitive?
	
\subsection{Strategies for Varying Workloads}

\subsubsection{Workload Characterization}
\challenge{Challenge: Hallo}


\subsubsection{Workload-aware Performance Modeling}
\challenge{Challenge: Hallo}

\subsubsection{Transfer Learning for Performance Models}
\challenge{Challenge: Hallo}
