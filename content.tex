...
Take as an example the method \texttt{insertRows} in Listing~\ref{lst:intro} from an imaginary database system. It receives as an argument an array of strings, each a row to insert. The method’s execution is influenced by two configuration options, \texttt{DUPLICATE\_CHECK} and  \texttt{AUTOCOMMIT}. If \texttt{DUPLICATE\_CHECK} is enabled, the passed array of insertion statements is checked for duplicates by  inserting all array items into a \texttt{HashSet} (\colorbox{duplicatecheck}{Lines 2--6}).
If \texttt{AUTOCOMMIT} is enabled, each insertion is treated as a single transaction (\colorbox{autocommit}{Lines 12–14}), wheres the method \texttt{commit} might otherwise be called  after  all rows have been inserted.
\begin{figure}
...
\end{subfigure}
	...
	\label{fig:intro}
\end{figure}
Besides, the execution of \texttt{insertRows} also depends on its workload,  the array of insertion statements. If the array size exceeds 50 (line~50), by design all insertions are handled by another method \texttt{insertBulkRows}. Consequently, the code section depending on option \texttt{AUTOCOMMIT} also depends on the workload size as it is skipped for arrays with more than 50 items. 
To now model the performance of any execution of the method \texttt{insertRows}, we consider the influence contributed by the configuration options, the workload, and the interactions between both. If we assume that each insertion takes a constant amount of time, we expect the method execution time to be proportional to the number of insertion statements. For the individual configuration options, however, the workload size can distort this conception. We illustrate this facet in Figure~\ref{fig:intro}, where the performance influence of both configuration options is shown as a function of the number of insertions. Here, the influence of option \texttt{AUTOCOMMIT} becomes negligible for workloads with more than 50 insertion statements. That is, any configuration  with this option enabled will behave differently depending on the workload. At large, a performance model for this database system learned with a workload of either less or more than 50 insertions cannot generalize to the respective other scenario.
	
In this paper, we shed light on whether and how workloads as a factor of variation influence the performance behavior of configurable software systems. We extend our perspective from pure black-box performance measurements under different configurations~\cite{dorn2020,siegmundPerformanceinfluenceModelsHighly2015,haDeepPerf2019,perfAL,guoVariabilityawarePerformancePrediction2013,sarkarCostEfficientSamplingPerformance,guo_2018_data,fourier_learning_2015,perLasso} with knowledge about the code executed. Specifically, we enrich performance measurements under different workloads and configurations with white-box statement coverage data. This allows us to draw a broader picture of multi-factor influences on software performance and paves an avenue to provide pointers for practitioners to select workloads that best represent the performance behavior of a software system and account for performance variation caused by workloads.	

{\color{red}In an empirical study of X configurations and Y workloads across Z configurable software systems, we found that \ldots 
	
In summary, we offer the following contributions:
\begin{itemize}

	\item An empirical study of the performance behavior of X configurations, Y workloads across Z configurable software systems.
...
\todo{clarify what a  performance distribution is}
Previous work has illustrated that performance variation can be attributed to differences in the workload. While the performance distributions of \textsc{x264} observed by \citeauthor{alves_sampling_2020} exhibit little to no similarity to each other~\cite{alves_sampling_2020}, for another series of subject systems, the influences of options (or their order) remained mostly stable, while only few shifted~\cite{jamshidi_transfer_gp_2017,jamishidi_transfer_2017}.  The similarity between workload-specific performance distributions observed by \citeauthor{jamshidi_learning_2018} is key to adapt existing performance models to a new workload~\cite{jamshidi_learning_2018}. 
...
Software performance emerges from a multitude of factors, of which configuration options have been widely studied and used for learning performance models. As workloads are, so far, an overlooked factor when learning performance models, we study the influence that workloads as an orthogonal factor, individually and in conjunction with configuration options, contribute to configurable software systems. In an empirical study of X configurations and Y workloads, we explore both how varying workloads affect the performance behavior and execution across Z software systems. 
