...
We limit the set of independent variables to individual options rather than to include higher-order interactions to be consistent with the feature location used for \RQref{3} where we determine option-specific, yet not interaction-specific code segments.
...
\circled{1} \emph{Spread} of performance influences: Some distributions scatter over a wide range, while others are concentrated around a single value. Consider \htwo, we observe for option \textsf{MVSTORE} a relative performance difference of 200\,\%, meaning that the performance impact of turning on this option can be twice as high depending on the workload. 

\circled{2} \emph{Opposing influences}: Some distributions exhibit both positive and negative coefficients, while others remain consistently positive or negative. Consider \dconvert, we observe both positive and negative influences for option \textsf{floor} for two workloads, while for the remaining workloads, the option has neglible influence.

\circled{3} \emph{Conditional influences}: For some distributions the majority of coefficients is negligible, but for few workloads we observe options having an influence. Consider \kanzi, we observe for option \textsf{BWT} we observe a negative influence only for a specific workload, whereas for all others the option has no influence.
...
\begin{table}[ht!]
	...
	\begin{tabular}{p{1.2cm}p{1.2cm}rp{4.3cm}} % h2 batik jadx
		\toprule
		\textbf{Category} & \textbf{Case Study} & \textbf{\#} & \textbf{Options} \\
		\midrule
		\multirow{5}{*}{\parbox{1.3cm}{\centering Conditional Influence}} & \jumper & 3 & \textsf{Mono, MidSideStereo, JointStereo}\\
		\cmidrule{2-4}
		...
		\multirow{5}{*}{\parbox{1.3cm}{\centering Opposing Influences}} & \kanzi & 2 & \textsf{Huffman, RLT} \\
		\cmidrule{2-4}
		...
		\multirow{5}{*}{\parbox{1.3cm}{ \centering High Spread}} & \kanzi & 1 & \textsf{TPAQX}\\
		\cmidrule{2-4}
...
\end{comment}
\vspace{2mm}
\greybox{\textbf{Summary} (\RQref{2}): Configuration options are profoundly input sensitiv: We observe high performance variantions, non-mo\-no\-to\-nic behavior, conditional influence, and even diametrically opposed influence for a single option.}
...
From the findings of the previous research questions, we have learned that input sensitivity of configuration options is specific to certain configuration options and can be diverse in the observed characteristics. With regard to the characteristic traits from~\RQref{2}, we conjecture that workload-specific sign flipping or conditional influences are driving factors for non-monotonicity across entire performance distributions (cf.~\RQref{1}). From these findings, the question arises what causes these different aspects of input sensitivity. For a practical setting, one might, in addition, ask whether it is possible to identify input-sensitive configuration options without an assessment resembling our case study.
...
\begin{comment}
{\color{orange}\todo{Kandidat zum Kürzebn}
