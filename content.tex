...
{\color{purple}We conceive a workload as any form of \textit{input fed to a software system for processing}. By nature, the workload of a software system is application-specific, such as a set of queries and transactions fed to a database system, a set of raw image files for video encoding or an arbitrary file for data compression etc. Each workload $w \in W$, similarly to configurable software systems, exhibits characteristics (or dimensions) that can be used to describe and quantify its properties, such as file size, file type, the type and composition of queries and so on. In performance engineering, workload characterization is a well-defined and widely employed task~\cite{} in order to reason about the performance of a software system and systematically assess software quality across a diverse set of workloads. 
While workload characterization is a well-defined task, one cannot guarantee that it yields the complete set of characteristics and therefor knowledge of the domain of possible workloads. Some characteristics can be non-trivial in that they might not be documented or known to developer, or only become relevant for performance under a specific configuration of a software system. Thus, from workload characterization, at best, one can assume an approximation of the workload domain model.}

\subsection{Workload-dependent Performance}
Workload-specific differences in the performance behavior of \textit{configurable} software systems can be triggered from only varying few characteristics, as the following example from our experiments demonstrate. In Figure~\ref{fig:h2_intro}, we show the performance distribution of {\color{red} XXXX configurations} of the software system \htwo when running two different parameterizations of the TPC-C benchmark. While the only parameter changed in this setting is the scale factor which accounts for the number of warehouses in the underlying schema, the differences in performance show both expectable as well as non-trivial traits. 

\begin{figure}[ht]
	\begin{subfigure}{0.48\linewidth}
		\includegraphics[width=\textwidth]{images/h2_tpcc-2.eps}
		\caption{Scale factor: 2}
		\label{fig:h2_intro_a}
	\end{subfigure}
	\begin{subfigure}{0.48\linewidth}
		\includegraphics[width=\textwidth]{images/h2_tpcc-8.eps}	
		\caption{Scale factor: 8}
		\label{fig:h2_intro_b}
	\end{subfigure}
	\caption{Performance distributions of \htwo ({\color{red} XXXX configurations}) and the TPC-C benchmark at different scale factors.}
	\label{fig:h2_intro}
	
\end{figure}

When increasing the complexity of the workload, one would expect the throughput to decrease, which is represented in the results: The minimum throughput in Figure~\ref{fig:h2_intro_a} is about 600, whereas the mode in Figure~\ref{fig:h2_intro_b} is about 600. By contrast, while the mean throughput decreases, we see that in Figure~Figure~\ref{fig:h2_intro} some confifgurations exceed the maximum throughput from Figure~\ref{fig:h2_intro_a}, i.e., some configurations did not follow the trend.Similar behavior has been documented before, for instance, \citeauthor{alves_sampling_2020} have observed dissimilar performance distributions for the video transcoder \textsc{x264} when proocessing two different  video files~\cite{alves_sampling_2020}. 


\subsection{Problem Statement}

\begin{equation}
	\Pi(c): \underbrace{\color{blue}\beta_1 c_1}_{\text{sensitive to w2}} + \beta_2 c_2 + ... + \beta_0
\end{equation}
...
\section{Results}
\subsection{Performance Across Workloads}\label{sec:rq1}
...
\section{Discussion} 
\subsection{Implications for Practicioners}
