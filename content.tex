...
\begin{table}[ht!]
	...
	\begin{tabular}{p{1.2cm}p{0.7cm}rp{4.9cm}} % h2 batik jadx
		...
		\cmidrule{2-4}
		 ...
		 \cmidrule{2-4}Varying the workload causes a substantial amount of variation among performance distributions. Across workloads, we observed \textit{mostly linear} (majority for for 70\,\% of the subject systems), but to a large extent, also \textit{non-monotonous} relationships (majority for 30\,\% of the subject systems).
...
\greybox{\textbf{Lesson 1:} Option-level input sensitivity can challenge robust and practical configuration-specific performance models, yet is neglected in state-of-the-art approaches.
}
...
\greybox{\textbf{Lesson 2:} Understanding the driving factors for input sensitivity is necessary for providing representative workloads and identifying sensitive configuration options. 
}
