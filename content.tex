...
{
	\color{red!65!black}
	Software performance (and other non-functional properties) can emerge from a variety of different factors. While there is a substantial body of work on modeling the relationshipü between configuration options and per
...
\todo{\small a discussion which precise performance counters have been considered, what aexplicitly does performance mean?}
...
For all data points, we report the median {\color{brown}performance} across five repetitions (except for \htwo), which has shown to be a good trade-off between variance and measurement effort~\cite{molyneauxArtApplicationPerformance2015}. 
...
\color{red!70!black}
\section{Exploring Causes of Workload Sensitivity}
\cite{lesoil_2021,samoaa_2021}
