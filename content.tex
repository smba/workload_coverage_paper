...
All experiments were conducted on clusters with identical hardware setup, either i7-... (cluster A) or \ldots (cluster B), all running a headless Ubuntu 18.04 installation. The machines had the minimal number of software packages installed and no additional user process running in the background to minimize measurement noise. 

All reported data points are the median across five repetitions (except for \sosy{h2} and \sosy{hsqldb}), which has shown to be a good trade-off between variance and measurement effort. The reported coefficient of variation was below {\color{Red}10\,\%. For the two database systems, we omitted the repetitions as in a pre-study, we found that across all benchmarks throughput variation was consistently below 5\,\%.}
...
\section{Estimating Representativeness (RQ2)}~\label{sec:metric}
\blindtext
