...
\subsubsection{Operationalization}To explore the possible driving factors of input sensitivity, we switch to the code level and analyze the relation of performance influences inferred for each configuration option (\RQref{1}) to code coverage information. Therefore, we augment our performance observations with code coverage information to assess differences in the execution under different workloads. Specifically, we are interested in code sections that implement option-specific functionality. From comparing the coverage information of option-specific code, we can formulate different hypothetical scenarios explaining input sensitivity. 
...
\subsection{Input Sensitivity and State-of-the-art Approaches}
...
\greybox{\textbf{Summary:} Input sensitivity challenges the robustness and generalizability configuration-specific performance models, yet is neglected in state-of-the-art approaches.
}
...
\greybox{\textbf{Summary:} Applying transfer learning to adapt performance models to new workloads requires a more reliable basis and assessment of input sensitivity. Domain-specific and workload-aware approaches are effective and should be extended on.
}
...
\greybox{\textbf{Summary:} Assessing input sensitivity remains an open challenge that requires a multi-method approach. Its benefits are likely to improve the reliability and practicality of performance prediction models. 
}
...
{\color{edited}We find that input sensitivity does not only affect software performance at the system-level, but affects individual configuration options as well, often in a non-monotonous way. We critically reflect on these patterns and aim at raising awareness to the missing notion of input sensitivity in state-of-the-art-approaches that model configuration-specific software performance. Our study provides an empirical basis which questions the practicality and generalizability of such approaches and calls for incorporating input sensitivity to maintain and improve the representativeness of approaches that model configuration-specific software performance.}
