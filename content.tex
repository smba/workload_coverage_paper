...
\itshape
The insights of RQ3, specifically, are that one could, in some scenarios, identify input sensitivity by systematic coverage testing. As an actionable, such practice can be easily incorporated into CI/CD pipelines, along with more advanced code analyses (e.g., hit counts). Hence, we may be able to enhance existing approaches with a low-cost metric. Further findings regarding novel benchmark designs for configurable software systems have been discussed above. We will make these findings more concise in the paper.

The results of RQ3 are important in two ways. First, we can estimate up-front whether an option is input-sensitive just by analyzing code-coverage data. Such information can be used to annotate existing performance models (e.g., by a utilization score per option) and help to interpret and contextualize performance estimations. Second, the results provide means for sound benchmark design of configurable software systems. That is, we lay the foundation of a metric that assesses the ‘representativeness’ of a workload by measuring which (parts of) options’ code are executed and to what extent.
