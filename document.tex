...
In this paper, we propose a lightweight approach that estimates potential bias of workload on different selectable software features. %We are using lightweight, dynamic methods known under the umbrella term \emph{software reconnaissance}
...
\begin{itemize}
	...
	\item \textsc{x264}~\cite{alves_sampling_2020}, varying workloads for prediction~\cite{liao_2020_using_emse} Recent work by Liao et al. illustrates that the potential bias of an individual workload can be averaged out by learning a prediction model on a set of workloads rather than only relying on a single one~\cite{liao_2020_using_emse}. By comparing prediction models learned from disjoint sets of workloads, the authors are able to detect performance regressions from one version of a software system to another.
\end{itemize}
