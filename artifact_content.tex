...
Take as an example the visualization of distribution of configuration options' influences across different workloads for the database system \htwo presented in Fig.~\ref{fig:dashboard} using the dashboard. 
...
framework \texttt{streamlit} and is provided as a \textsc{Docker} container via the companion repository. This way, the dashboard not only complements the presentation of results in the study, but provides a means to reproduce analyses and reenact findings from our study.
